...
Set `simplify = FALSE` to get the raw API response as a nested list
instead.
...
`lei_mapping()` downloads the latest bulk relationship file that maps
every LEI to another identifier type: ISIN, BIC, MIC, or OpenCorporates
ID (`"oc"`):
...
`lei_regions()` and `lei_issuers()` return supporting reference data,
the latter listing the accredited Local Operating Units (LOUs) that
issue LEIs:
